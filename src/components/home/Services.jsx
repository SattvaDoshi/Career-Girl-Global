...
const Services = () => {
  ...
  return (
    <div
      ref={sectionRef}
      className="flex flex-col items-center min-h-screen bg-pink-100 p-8"
          ...
          >
            <div>
              <img
                src={card.image}
                alt={card.title}
                style={card.title === 'E-Book' ? { width: '400px', height: '200px', marginTop: '40px' } : {}}
              />
            </div>
            <h2 className="text-2xl font-bold text-pink-600 mt-6 mb-4">{card.title}</h2>
            <p className="text-center text-gray-700 mb-6">{card.description}</p>
            <Link
              to={card.link}
              className="bg-pink-500 text-white py-2 px-6 rounded-full hover:bg-pink-600 transition-colors duration-300"
            >
              Get Started
            </Link>
          </div>
